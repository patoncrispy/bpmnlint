...
Linter.prototype.parseRuleName = function(name) {
  ...
  if (name.startsWith('@')) {
    ...
    if (!Array.isArray(match) || !match.length) {
      throw new Error(`invalid config name <${name}>. Must pass regex /^(?:@[a-z0-9-~][a-z0-9-._~]*\\/)?bpmnlint-plugin-[a-z0-9-._~]+/gm`);
    }
...
Linter.prototype.parseConfigName = function(name) {
  ...
  if (name.toLowerCase().startsWith('plugin:@')) {
    const slashIdx = name.lastIndexOf('/');
    const pkgRegex = /(?:@[a-z0-9-~][a-z0-9-._~]*\/)?bpmnlint-plugin-[a-z0-9-._~]+/gm;
    const configName = name.substring(slashIdx + 1);
    const match = name.match(pkgRegex);
    if (!Array.isArray(match) || !match.length) {
      throw new Error(`invalid package name <${name}>. Must pass regex /(?:@[a-z0-9-~][a-z0-9-._~]*\\/)?bpmnlint-plugin-[a-z0-9-._~]+/gm`);
    }
    ...
    throw new Error(`invalid config name <${name}>. Must pass regex /^plugin:(?:@[a-z0-9-~][a-z0-9-._~]*\\/)?bpmnlint-plugin-[a-z0-9-._~]+\\/[\\w]+/gm`);
  }
